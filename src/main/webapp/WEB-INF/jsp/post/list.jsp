...
<div class="mt-5">
		<div class="d-flex ml-5" >			
			...
			<button type="button" class="beige-color post-list-btn black-color" id= "popularityBtn" data-sort-id="2">공감순 
		...
		$("#latelyBtn").on('click', function(e){
		 ...
		 $("#popularityBtn").on('click', function(e){
			...
			$.ajax({
				...
				, success:function(){
						// alert("공감순 정렬 성공");
						location.href="/post/list-view/2"; 
				}	
				, error:function(request, status, error) {
					alert("공감순으로 정렬하는데 실패했습니다.");
				}
		...
		$("#commentBtn").on('click', function(e){
			...
			$.ajax({
				...
				, success:function(){
						// 성공
						// alert("댓글순 정렬 성공")
						location.href="/post/list-view/3";
				}	
				, error:function(request, status, error) {
					alert("댓글순으로 정렬하는데 실패했습니다.");
				}	
		}); // ---- popularityBtn 
	}); 
});	
